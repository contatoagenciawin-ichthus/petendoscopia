...
export default async function CampaignPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main><CampaignTracker campaign={slug} />
    ...
    <section className="campaign-hero"><div><p className="eyebrow">{campaign.eyebrow}</p><h1>{campaign.title}</h1><p className="lead">{campaign.description}</p><div className="actions"><TrackedLink className="button" href={wa} target="_blank" rel="noreferrer" event="whatsapp_click" data={{ source: slug, cta: "hero" }}>Solicitar orientação</TrackedLink><a className="button button-outline" href="#contato">Enviar dados do caso</a></div></div><Image src={slug === "para-veterinarios" ? "/images/equipe-procedimento.webp" : "/hero-pet-endoscopia.webp"} alt="Equipe Pet Endoscopia em procedimento veterinário" width={1000} height={760} priority unoptimized /></section>
